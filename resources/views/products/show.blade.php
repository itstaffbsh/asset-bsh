<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-serif text-2xl font-bold text-[#4a554a] leading-tight">{{ __('Detail Aset') }}: {{ $product->description }}</h2>
            <div class="flex gap-2">
                <a href="{{ route('products.qr', $product->id) }}" class="bg-[#5c6b5b] text-white px-4 py-2 rounded-md shadow hover:bg-[#4a554a] transition">📱 {{ __('Cetak QR') }}</a>
                <a href="{{ route('products.index') }}" class="text-[#8b6540] hover:underline flex items-center">← {{ __('Kembali') }}</a>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-[#e5e0d8] overflow-hidden">
                    ...
                    <div class="p-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-12">
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('ID Barang / Nomor Unik') }}</p>
                                <p class="text-lg font-mono font-bold text-[#5c6b5b]">{{ $product->full_nomor_unik }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('Serial Number (S/N)') }}</p>
                                <p class="text-lg font-mono text-gray-700">{{ $product->serial_number ?? '-' }}</p>
                            </div>
                            <div class="md:col-span-2">
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('Processor') }}</p>
                                <p class="text-lg text-[#4a554a] font-semibold">{{ $product->processor ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('RAM (Total / New)') }}</p>
                                <p class="text-lg text-[#4a554a] font-semibold">{{ $product->ram ?? '-' }} <span class="text-sm text-green-600 font-normal">({{ __($product->new_ram ?? 'No Upgrade') }})</span></p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('SSD (Total / New)') }}</p>
                                <p class="text-lg text-[#4a554a] font-semibold">{{ $product->ssd ?? '-' }} <span class="text-sm text-green-600 font-normal">({{ __($product->new_ssd ?? 'No Upgrade') }})</span></p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('Operating System') }}</p>
                                <p class="text-base text-gray-600">{{ $product->os ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('Device Class') }}</p>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-[#e5e0d8] overflow-hidden">
                    <div class="px-6 py-4 border-b border-[#e5e0d8] bg-[#f9f8f6]">
                        <h3 class="font-serif font-bold text-[#4a554a] flex items-center gap-2">
                            <span>📡</span> {{ __('Konektivitas & Identitas') }}
                        </h3>
                    </div>
                    <div class="p-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('MAC Address') }}</p>
                                <p class="font-mono text-sm">{{ $product->mac_address ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('IMEI') }}</p>
                                <p class="font-mono text-sm">{{ $product->imei ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('Screen ID') }}</p>
                                <p class="text-sm">{{ $product->screen_id ?? '-' }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-widest mb-1">{{ __('Harga Perolehan') }}</p>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-[#5c6b5b] text-white rounded-xl shadow-lg p-8 relative overflow-hidden">
                    <div class="relative z-10">
                        <p class="text-xs text-[#c8dfc6] uppercase tracking-[0.2em] mb-4">{{ __('Pemegang Saat Ini') }}</p>
                        ...
                        @else
                            <div class="py-8 text-center">
                                ...
                                <h4 class="text-xl font-serif font-bold">{{ __('Tersedia di Kantor') }}</h4>
                                <p class="text-sm text-[#c8dfc6] mt-2 italic">{{ __('Belum ada pemegang aktif') }}</p>
                            </div>
                        @endif
                        ...
                        <div class="mt-8">
                            <a href="{{ route('products.' . ($currentHistory && $currentHistory->diterima_oleh ? 'kembali' : 'pinjam'), ['q' => $product->full_nomor_unik]) }}" 
                               ...
                               {{ $currentHistory && $currentHistory->diterima_oleh ? '🔄 ' . __('Proses Pengembalian') : '🤝 ' . __('Proses Peminjaman') }}
                            </a>
                        </div>
                    </div>
                    {{-- Dekorasi background --}}
                ...
                <div class="bg-white rounded-xl shadow-sm border border-[#e5e0d8] p-6 text-center">
                    <p class="text-xs text-gray-400 uppercase tracking-widest mb-2">{{ __('Departemen Pemilik') }}</p>
                    <h4 class="text-2xl font-serif font-bold text-[#4a554a]">{{ $product->department->nama_departemen }}</h4>
                    <p class="text-sm text-gray-500 mt-1">{{ __('Kode Asset') }}: <span class="font-mono font-bold">{{ $product->department->kode_asset }}</span></p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-[#e5e0d8] overflow-hidden">
            <div class="px-6 py-4 border-b border-[#e5e0d8] bg-[#f9f8f6]">
                <h3 class="font-serif font-bold text-[#4a554a] flex items-center gap-2">
                    <span>📜</span> {{ __('Riwayat Mutasi Aset') }}
                </h3>
            </div>
            <div class="p-0">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-100">
                            <th class="py-3 px-6 text-xs font-bold text-gray-400 uppercase">{{ __('Tanggal') }}</th>
                            <th class="py-3 px-6 text-xs font-bold text-gray-400 uppercase">{{ __('Aksi') }}</th>
                            <th class="py-3 px-6 text-xs font-bold text-gray-400 uppercase">{{ __('Penanggung Jawab') }}</th>
                            <th class="py-3 px-6 text-xs font-bold text-gray-400 uppercase">{{ __('Penerima / Lokasi') }}</th>
                            <th class="py-3 px-6 text-xs font-bold text-gray-400 uppercase">{{ __('Status') }}</th>
                            <th class="py-3 px-6 text-xs font-bold text-gray-400 uppercase text-center">{{ __('STTB') }}</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="py-4 px-6">
                                    @if($history->diterima_oleh)
                                        <span class="text-xs text-blue-600 italic">{{ __('Peminjaman Aktif') }}</span>
                                    @else
                                        <span class="text-xs text-green-600 italic">{{ __('Tersedia di Kantor') }}</span>
                                    @endif
                                </td>
