    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div id="import-modal-asset" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white p-8 rounded-xl shadow-xl w-96">
                <h3 class="text-lg font-bold mb-4">{{ __('Import Data Aset') }}</h3>
                <form action="{{ route('products.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="file" name="file" class="mb-4 w-full" required>
                    <div class="flex justify-end gap-2">
                        <button type="button" onclick="document.getElementById('import-modal-asset').classList.add('hidden')" class="px-4 py-2 text-gray-500">{{ __('Batal') }}</button>
                    ...
                    <form action="{{ route('products.index') }}" method="GET" id="filter-form-assets">
                        <div class="flex flex-wrap gap-3 items-end">
                            ...
                            <div class="flex-1 min-w-[200px]">
                                <label class="block text-xs font-bold text-[#5c6b5b] uppercase tracking-widest mb-1">{{ __('Cari Aset') }}</label>
                                <div class="relative">
                            ...
                            {{-- Filter Kategori --}}
                            <div class="min-w-[160px]">
                                <label class="block text-xs font-bold text-[#5c6b5b] uppercase tracking-widest mb-1">{{ __('Kategori') }}</label>
                            ...
                            <div class="flex gap-2">
                                ...
                                @if($searchTerm || $filterDept || $filterKantor || $filterKategori)
                                    <a href="{{ route('products.index') }}" class="bg-red-50 text-red-500 border border-red-200 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-100 transition">✕ {{ __('Reset') }}</a>
                                @endif
                            </div>
                        </div>
                    </form>
                    ...
                    <table class="w-full text-left border-collapse min-w-[1500px]">
                        <thead>
                            <tr class="bg-[#f9f8f6] border-b border-[#e5e0d8]">
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Employee ID') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Name') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Category') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Description') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('ID Barang') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Processor') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('RAM') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('SSD') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('OS') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('S/N') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('MAC Address') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('IMEI') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Screen ID') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Harga') }}</th>
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b] text-xs uppercase tracking-wider">{{ __('Class') }}</th>
                        ...
                        <tbody>
                            ...
                            @endforeach
                            @if($products->isEmpty())
                            <tr><td colspan="10" class="py-8 text-center text-gray-500 italic">{{ __('Belum ada data aset.') }}</td></tr>
                            @endif
                        </tbody>
                    </table>
                </div>
                ...
                <form id="deleteForm" method="POST">
                    ...
                    <div>
                        ...
                        <div class="mt-3 text-center sm:mt-5">
                            <h3 class="text-lg font-medium leading-6 text-gray-900" id="modal-title">{{ __('Hapus Aset') }}</h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">{{ __('Anda akan menghapus aset') }} <span id="deleteAssetName" class="font-bold"></span>. {{ __('Silakan pilih alasan penghapusan') }}:</p>
                            </div>
                        </div>
                          <div class="mt-5 space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700">{{ __('Alasan Penghapusan') }}</label>
                            <select name="deletion_reason" id="deletion_reason" onchange="toggleSellingPrice()" class="block w-full mt-1 border-gray-300 rounded-md shadow-sm focus:ring-[#5c6b5b] focus:border-[#5c6b5b] sm:text-sm" required>
                                <option value="dihancurkan">{{ __('Dihancurkan (Scrapped)') }}</option>
                                <option value="dijual">{{ __('Dijual (Sold)') }}</option>
                            </select>
                        </div>
                        <div id="sellingPriceContainer" class="hidden">
                            <label class="block text-sm font-medium text-gray-700">{{ __('Harga Jual (Rp)') }}</label>
                            <div class="flex mt-1 rounded-md shadow-sm">
                                ...
                                <input type="text" name="selling_price" id="selling_price" class="flex-1 block w-full border-gray-300 rounded-none rounded-r-md focus:ring-[#5c6b5b] focus:border-[#5c6b5b] sm:text-sm" placeholder="{{ __('Contoh: 500.000') }}">
                            </div>
                        </div>
                    </div>                </div>
                    <div class="mt-5 sm:mt-6 sm:grid sm:grid-cols-2 sm:gap-3 sm:grid-flow-row-dense">
                        <button type="submit" class="inline-flex justify-center w-full px-4 py-2 text-base font-medium text-white bg-red-600 border border-transparent rounded-md shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:col-start-2 sm:text-sm">{{ __('Hapus (Backup)') }}</button>
