            ...
            <form action="{{ route('employees.store') }}" method="POST" class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="employee_id" value="{{ old('employee_id') }}" class="w-full border-[#d1cdba] rounded-lg shadow-sm focus:border-[#a47b53]">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-[#5c6b5b] mb-1">{{ __('Nama Lengkap') }} <span class="text-red-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name') }}" required class="w-full border-[#d1cdba] rounded-lg shadow-sm focus:border-[#a47b53]">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-[#5c6b5b] mb-1">{{ __('Email') }} <span class="text-red-500">*</span></label>
                        <input type="email" name="email" value="{{ old('email') }}" required class="w-full border-[#d1cdba] rounded-lg shadow-sm focus:border-[#a47b53]">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-[#5c6b5b] mb-1">{{ __('Nomor HP') }}</label>
                        <input type="text" name="phone_number" value="{{ old('phone_number') }}" class="w-full border-[#d1cdba] rounded-lg shadow-sm focus:border-[#a47b53]">
                    </div>
                    <div>
                        ...
                        <select name="department_id" class="w-full border-[#d1cdba] rounded-lg shadow-sm focus:border-[#a47b53]">
                            ...
                            @foreach($departments as $d)
                                <option value="{{ $d->id }}" {{ old('department_id') == $d->id ? 'selected' : '' }}>{{ $d->nama_departemen }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="office_id" required class="w-full border-[#d1cdba] rounded-lg shadow-sm focus:border-[#a47b53]">
                            ...
                            @foreach($offices as $o)
                                <option value="{{ $o->id }}" {{ old('office_id') == $o->id ? 'selected' : '' }}>{{ $o->nama_kantor }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="date" name="join_date" value="{{ old('join_date') }}" class="w-full border-[#d1cdba] rounded-lg shadow-sm focus:border-[#a47b53]">
                    </div>
                </div>

                <div class="mt-8 flex justify-end">
                    <button type="submit" class="bg-[#5c6b5b] text-white px-6 py-2 rounded-lg font-bold hover:bg-[#4a554a] transition shadow-md">
                        {{ __('Simpan Data Karyawan') }}
                    </button>
                </div>
            </form>
        </div>
