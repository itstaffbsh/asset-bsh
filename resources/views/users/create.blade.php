<x-app-layout>
    <x-slot name="header"><h2 class="font-serif text-2xl font-bold text-[#4a554a] leading-tight">{{ __('Tambah Akun Karyawan') }}</h2></x-slot>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-[#e5e0d8] p-8">
            ...
            <form action="{{ route('accounts.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                    <div>
                        <label class="block text-[#5c6b5b] text-xs font-bold uppercase tracking-widest mb-2">{{ __('Employee ID (Kosongkan untuk otomatis)') }}</label>
                ...
                <div id="password-container" class="mb-5">
                    <label class="block text-[#5c6b5b] text-xs font-bold uppercase tracking-widest mb-2">{{ __('Password Login') }}</label>
                    <input type="password" name="password" class="w-full border-[#d1cdba] rounded-md shadow-sm focus:border-[#a47b53] focus:ring-[#a47b53] focus:ring-opacity-20" placeholder="{{ __('Minimal 8 karakter') }}">
                </div>
                ...
                <div class="flex items-center gap-4">
                    <button type="submit" class="bg-[#5c6b5b] text-white px-8 py-3 rounded-lg shadow-lg hover:bg-[#4a554a] transition font-bold uppercase tracking-wider text-sm">{{ __('Simpan Akun') }}</button>
    ...
    <script>
        function togglePassword() {
            ...
            // Di sistem baru, hampir semua role butuh password login.
            // Kita bisa sembunyikan hanya jika role adalah 'user' (jika masih ada role slug user)
            if (slug === 'user') {
