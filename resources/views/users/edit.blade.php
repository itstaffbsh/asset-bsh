<x-app-layout>
    <x-slot name="header"><h2 class="font-serif text-2xl font-bold text-[#4a554a] leading-tight">Edit Akun Karyawan</h2></x-slot>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-[#e5e0d8] p-8">
            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                    <div>
                        <label class="block text-[#5c6b5b] text-xs font-bold uppercase tracking-widest mb-2">Employee ID</label>
                ...
                <div id="password-container" class="{{ in_array(old('role', $user->role), ['admin', 'super_admin']) ? '' : 'hidden' }} mb-5">
                    <label class="block text-[#5c6b5b] text-xs font-bold uppercase tracking-widest mb-2">Ganti Password</label>
                    <input type="password" name="password" class="w-full border-[#d1cdba] rounded-md shadow-sm focus:border-[#a47b53] focus:ring-[#a47b53] focus:ring-opacity-20" placeholder="Kosongkan jika tidak ingin ganti">
                </div>
                ...
                <div class="flex items-center gap-4">
                    <button type="submit" class="bg-[#5c6b5b] text-white px-8 py-3 rounded-lg shadow-lg hover:bg-[#4a554a] transition font-bold uppercase tracking-wider text-sm">Perbarui Akun</button>
