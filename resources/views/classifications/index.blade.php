<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('classifications.create') }}" class="bg-[#5c6b5b] text-[#f4f1ea] px-4 py-2 rounded-md shadow hover:bg-[#4a554a] transition duration-200">Tambah Klasifikasi</a>
        </div>
    </x-slot>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-[#f9f8f6] border-b border-[#e5e0d8]">
                                <th class="py-3 px-4 font-semibold text-[#5c6b5b]">Nama Klasifikasi</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-[#f4f1ea] hover:bg-[#faf9f7] transition duration-150">
                                ...
                                <td class="py-3 px-4 text-center">
                                    <a href="{{ route('classifications.edit', $classification->id) }}" class="text-[#a47b53] hover:text-[#8b6540] mr-3 font-medium">Edit</a>
                                    <form action="{{ route('classifications.destroy', $classification->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus klasifikasi ini?');">
                                        @csrf
                                        @method('DELETE')
                            ...
                            <tr>
                                <td colspan="2" class="py-8 text-center text-gray-500 italic">Belum ada data klasifikasi.</td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
